...
expr:
	...
	| 'try' '{' tryExpr = expr '}' 'cast' 'as' type_ = stellatype '{' pattern_ = pattern '=>' expr_
		= expr '}' 'with' '{' fallbackExpr = expr '}'					# TryCastAs
	...
	| fun = expr '[' (
		types += stellatype (',' types += stellatype)*
	) ']' # TypeApplication
	// expr
	...
	| 'generic' '[' generics += StellaIdent (
		',' generics += StellaIdent
	)* ']' expr_ = expr		# TypeAbstraction
